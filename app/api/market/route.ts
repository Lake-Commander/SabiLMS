...
export async function GET(request: Request) {
  ...
  //  Safe Neo-Brutalist backup mock set to keep student workspaces running during weekends/maintenance
  const BACKUP_STOCKS = [
  ...
  try {
    //  PATH A: If the client requests a specific stock's historical candlestick data timeline
    if (symbol) {
      const chartUrl = `https://api.ngnmarket.com/v1/companies/${symbol.toUpperCase()}/chart?period=${period}&format=ohlcv`;
      const chartRes = await fetch(chartUrl, {
        headers: { 'Authorization': `Bearer ${apiKey}` },
        next: { revalidate: 1800 } // Cache time-series data for 30 minutes
      });
      const chartPayload = await chartRes.json();
      
      // NGN Market wraps historical output directly inside payload.data array
    ...
    // 🏛️ PATH B: Default workspace payload load (Macro indicators + All Companies indices list)
    ...
    // Safely look inside data fields following response format rules
    const rawStocks = companiesPayload.success && Array.isArray(companiesPayload.data) ? companiesPayload.data : [];
    ...
    console.error("🚨 Next.js NGNMarket Data Proxy Layer Exception Error:", error);
    return NextResponse.json({
      success: false,
